...
Deploys a GitLab Runner with **Docker API compatibility** using Podman as the container runtime. Provides a `/var/run/docker.sock` symlink so existing Docker workflows work unchanged.
...
| <a name="input_gitlab_config_environment"></a> [gitlab\_config\_environment](#input\_gitlab\_config\_environment) | List of environment variables to set in the GitLab Runner manager container (e.g., for GitLab API access or custom credential helpers), in the form KEY=value | `list(string)` | `[]` | no |
| <a name="input_gitlab_config_pre_get_sources_script"></a> [gitlab\_config\_pre\_get\_sources\_script](#input\_gitlab\_config\_pre\_get\_sources\_script) | Optional script to run before the 'get sources' step of each job, for custom authentication to GitLab or other setup tasks. Should be idempotent and non-destructive, as it will run on every job execution | `string` | `""` | no |
...
| <a name="input_privileged_mode"></a> [privileged\_mode](#input\_privileged\_mode) | Enable Docker privileged mode for runners. Required for Docker-in-Docker (DinD) builds. WARNING: grants near-complete host kernel access to job containers | `bool` | `true` | no |
