...
variable "gitlab_config_environment" {
  type        = list(string)
...
variable "gitlab_config_pre_get_sources_script" {
  type        = string
...
variable "runner_environment" {
  type        = list(string)
  description = "Environment variables to set for the runner"
  default     = ["CONTAINER_HOST=unix:///var/run/docker.sock", "DOCKER_HOST=unix:///var/run/docker.sock"]
}
