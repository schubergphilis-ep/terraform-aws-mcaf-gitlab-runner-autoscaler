  ...
  gitlab_runner_config = {
    ...
    runners = {
      ...
      docker = {
        ...
        volumes    = ["/var/run/docker.sock:/var/run/docker.sock:z", "/builds", "/cache"]
      }
      autoscaler = {
        plugin                = "aws"
...
# Uses Podman in rootful mode with Docker socket compatibility symlink.
# Podman exposes a Docker-compatible API socket at /var/run/docker.sock,
# allowing CI jobs to use standard Docker CLI commands (docker build, push, etc.)
module "ignition" {
