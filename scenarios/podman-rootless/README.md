...
Deploys a GitLab Runner with **rootless Podman** as the container runtime. Provides enhanced security by running containers without root privileges.
...
- **No Docker-in-Docker**: Use [Kaniko](https://github.com/GoogleContainerTools/kaniko) or Buildah instead (see [building container images](../README.md#building-container-images))
- **Port binding**: Ports < 1024 require special configuration
- **Volume permissions**: Some volume mounts may need `:z` SELinux label
...
| <a name="input_privileged_mode"></a> [privileged\_mode](#input\_privileged\_mode) | Enable Docker privileged mode for runners. Defaults to false for rootless Podman since this scenario is designed for reduced-privilege operation | `bool` | `false` | no |
